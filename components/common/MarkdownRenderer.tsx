...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, className = '' }) => {
  ...
  const processedContent = content
    ...
    .replace(/\\quad/g, '  ')
    .replace(/\\textsf\{([^}]+)\}/g, '$1')
    // Aggressively remove rogue bullets before major technical labels (including unicode bullets like •)
    .replace(/^[*-•]\s+\*\*(Նկարագրություն|Նորմ|Կետ|Կարգավիճակ|Հղում|Լուծում|Առաջարկ|Խնդիր|Բացատրություն)\.\*\*/gm, '**$1.**')
    .replace(/^\s*[*-•]\s*$/gm, '') // Remove bullet-only lines
    ...
    <div className={`markdown-body book-style-report ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          ul: ({ node, ...props }) => <ul className="list-disc list-inside mb-1 space-y-0.5 ml-4" {...props} />,
          ol: ({ node, ...props }) => <ol className="list-decimal list-inside mb-1 space-y-0.5 ml-4" {...props} />,
