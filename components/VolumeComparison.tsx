...
const VolumeComparison: React.FC<{ subscriptionActive: boolean }> = ({ subscriptionActive }) => {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      // Select blocks to avoid slicing inside them (rows, headers, summary blocks)
      const blocks = Array.from(element.querySelectorAll('tr, .bg-white.rounded-2xl.p-6, h3'));
