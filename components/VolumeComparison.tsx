...
const VolumeComparison: React.FC<{ subscriptionActive: boolean }> = ({ subscriptionActive }) => {
  ...
  const exportPDF = async () => {
    ...
    doc.text("Ծավալների Համեմատություն", 14, 15);

    const tableData = result.comparisonTable.map(row => [
      row.itemName,
      row.unit,
      row.plannedQty.toFixed(2),
      row.actualQty.toFixed(2),
      (row.actualQty - row.plannedQty).toFixed(2)
    ]);

    (autoTable as any)(doc, {
      startY: 25,
      head: [['Անվանում', 'Չ/Մ', 'Նախ.', 'Փաստ.', 'Շեղում']],
      body: tableData,
      styles: { font: "helvetica", fontSize: 8 },
      headStyles: { fillColor: [37, 99, 235] }
    });

    doc.save(`Ծավալների_Համեմատություն_${new Date().toLocaleDateString()}.pdf`);
  };
