...
export const performAudit = async (request: AuditRequest, disciplines: PriorityTask[], auditQuestion: string): Promise<AuditResponse> => {
  ...
  const prompt = `
    ...
    **ԿԱՐԵՎՈՐ ՊԱՀԱՆՋ**: Ինֆորմացիան պետք է լինի բարձր մակարդակով համակարգված և պրոֆեսիոնալ:
    ...
    Օգտագործիր հետևյալ ԿԱՐԳԱՎՈՐՎԱԾ ձևաչափը յուրաքանչյուր կետի համար.

    ## [Ուղղության կամ Խնդրի Անվանումը]
    ***
    **Կետ №1․ [Վերնագիր]**
    *   🚩 **Կարգավիճակ/Նկարագրություն:** [Մանրամասն բացատրություն (Համապատասխանում է թե Խախտում է)]
    *   📚 **Նորմատիվ հղում:** [ՀՀՇՆ, գլուխ, կետ]
    *   💡 **Լուծման առաջարկ/Եզրակացություն:** [Շտկման քայլեր կամ ամփոփում]
    ***

    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ**:
    - **Մի օգտագործիր աղյուսակներ:** Դրանք անընթեռնելի են:
...
export const performCustomTask = async (request: AuditRequest, task: string): Promise<string> => {
  ...
  const prompt = `
    ...
    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ ՑՈՒՑՈՒՄՆԵՐ ԳՐՔԱՅԻՆ ՏԵՍՔԻ ՀԱՄԱՐ**:
    ...
    Օգտագործիր հետևյալ կառուցվածքը յուրաքանչյուր կետի համար.
    ## [Հարցի/Թեմայի Վերնագիր]
    ***
    **Կետ №1․ [Վերնագիր]**
...
export const performLayoutAudit = async (request: AuditRequest, question: string): Promise<string> => {
  ...
  const prompt = `
    ...
    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ ՑՈՒՑՈՒՄՆԵՐ ԳՐՔԱՅԻՆ ՏԵՍՔԻ ՀԱՄԱՐ**:
    ...
    Օգտագործիր հետևյալ կառուցվածքը յուրաքանչյուր կետի համար.
    ## [Վերլուծության Վերնագիր]
    ***
    **Կետ №1․ [Վերնագիր]**
    *   🚩 **Նկարագրություն:** [Մանրամասն բացատրություն]
    *   📚 **Նորմատիվ հղում:** [ՀՀՇՆ, գլուխ, կետ (եթե կա)]
    *   💡 **Առաջարկ:** [Ինչպես բարելավել]
    ***
  `;
