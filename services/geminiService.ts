...
export const performAudit = async (request: AuditRequest, disciplines: PriorityTask[], auditQuestion: string): Promise<AuditResponse> => {
  ...
  const prompt = `
    ...
    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ (Textbook Style)**:
    - Բոլոր հաշվարկային թվերը, բանաձևերը և միավորները ներկայացրու **LaTeX** ձևաչափով (օրինակ՝ $3000 \, \text{մ}^3/\text{ժ}$, $250 \, \text{Պա}$, $P_{total} = \sum \Delta P + P_{dyn}$):
    - Թվերը կլորացրու ՄԻԱՅՆ 2 տասնորդական նիշով:
    - Օգտագործիր աղյուսակներ (|---|) ՄԻԱՅՆ տեխնիկական պարամետրերի համեմատության համար:
    - Յուրաքանչյուր նոր թեմա կամ բաժին սկսիր **նոր տողից**։
    - Պատասխանը պետք է լինի խիստ պրոֆեսիոնալ և ինժեներական:
  `;
...
export const performCustomTask = async (request: AuditRequest, task: string): Promise<string> => {
  ...
  const prompt = `
    ...
    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ (Textbook Style)**:
    - Բոլոր հաշվարկային թվերը, բանաձևերը և միավորները ներկայացրու **LaTeX** ձևաչափով (օրինակ՝ $3000 \, \text{մ}^3/\text{ժ}$, $P_{total} = \sum \Delta P + P_{dyn}$):
    - Կետերը սկսիր ԲՈԼԴ վերնագրով՝ առանց բուլետների։
    - Բոլոր թվերը կլորացրու մինչև 2 տասնորդական նիշ:
    - Չափման միավորները գրիր LaTeX-ի մեջ հայերեն տեքստով:
  `;
...
`
  ...
  const prompt = `
    ...
    **Կարևոր դիտարկում.** [Ընդհանուր նկարագրություն]
    ...
    | Խնդիր | Լուծում | Հղում |
    | :--- | :--- | :--- |
    | [Խնդրի նկարագրությունը] | [Առաջարկվող լուծումը] | [Նորմատիվ հղումը] |
    ...
    🏁 Եզրակացություն և Հաջորդ Քայլեր
    [Ամփոփում և պահանջներ նախագծողից]
...
export const performLayoutAudit = async (request: AuditRequest, question: string): Promise<string> => {
  ...
  const prompt = `
    ...
    **ԾԱՅՐԱՀԵՂ ԿԱՐԵՎՈՐ (Textbook Style)**:
    - Բոլոր հաշվարկային թվերը, բանաձևերը և միավորները ներկայացրու **LaTeX** ձևաչափով (օրինակ՝ $3000 \, \text{մ}^3/\text{ժ}$):
    - Կետերը սկսիր ԲՈԼԴ վերնագրով՝ առանց բուլետների։
    - Բոլոր թվերը կլորացրու մինչև 2 տասնորդական նիշ:
    - Յուրաքանչյուր նոր թեմա սկսիր նոր տողից։
  `;
